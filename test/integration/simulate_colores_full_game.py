...
Usage:
    ...
    # Or run directly:
    python3 simulate_colores_full_game.py
"""
...
"""
    ...
    def _write_index(self) -> None:
        ...
        phases = ["P1", "P2", "P3", "P4_YESNO", "P6", "P7"]
        for phase in phases:
    ...
    def _simulate_phase(self, phase: str, timeout: float) -> None:
        ...
        # Wait for phase intro
        self.wait_for_decision_state(
            lambda s: s.get("gameState") == "PHASE_INTRO" and s.get("payload", {}).get("phase") == phase,
            timeout
        )
        ...
        while True:
            ...
            # Wait for question present
            try:
                self.wait_for_decision_state(
                    lambda s: s.get("gameState") == "QUESTION_PRESENT",
                    timeout=5.0
                )
            except TimeoutError:
            ...
            question = self._latest_decision_state.get("payload", {}).get("question", {})
            ...
            # Wait for WAIT_INPUT
            self.wait_for_decision_state(
                lambda s: s.get("gameState") == "WAIT_INPUT",
                timeout
            )
            time.sleep(0.3)
            ...
            if wrong_answer and phase != "P6":  # P6 doesn't have wrong answers
                ...
                # Wait for FAIL_L1
                self.wait_for_decision_state(
                    lambda s: s.get("gameState") == "FAIL_L1",
                    timeout
                )
                time.sleep(0.3)
            ...
            # Wait for CORRECT
            self.wait_for_decision_state(
                lambda s: s.get("gameState") == "CORRECT",
                timeout
            )
            time.sleep(0.3)
    ...
    def _get_phase_intro_text(self, phase: str) -> str:
        ...
        intros = {
            ...
            "P4_YESNO": "Responde sí o no a las preguntas.",
            "P6": "Dime qué color quieres señalar.",
            "P7": "¿Cuál es el color correcto?"
        }
